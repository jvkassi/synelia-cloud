...
import { UNIVERS_CLIENT, UNIVERS_SUPER_ADMIN, gabarit, panneauEspaceActif } from '@/lib/navigation'
...
/**
 * Coquille de contenu de l'espace super admin.
 *
 * Un seul univers y occupe toute la largeur : **Clients**, qui monte un panneau
 * listant les organisations. Même raison que Web Cloud — le panneau doit
 * toucher le bord de l'écran, et les tableaux d'une organisation sont larges.
 * Le reste de l'espace reste borné : ce sont des écrans de lecture.
 *
 * Comme côté client, le découpage est déclaré dans `lib/navigation.ts` plutôt
 * que décidé ici.
 */
export function ConteneurAdmin({ children }: { children: React.ReactNode }) {
  const pathname = usePathname()
  const forme = gabarit(UNIVERS_SUPER_ADMIN, pathname)
  if (forme === 'plein') return <>{children}</>
