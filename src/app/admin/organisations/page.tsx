...
import { ELEVATIONS, EQUIPE_SYNELIA, IMPAYES, ORGANISATIONS, RESELLERS, USERS } from '@/lib/mock'
...
export default function Organisations() {
  ...
  const [typeRelation, setTypeRelation] = useState<Organisation['type']>('direct')
    ...
    executer({
      ...
      effet: () =>
        orgs.creer({
          ...
          type: typeRelation,
          resellerId: typeRelation === 'client_revendeur' ? revendeur || undefined : undefined,
          statut: 'active',
  ...
  const viaRevendeur = orgs.items.filter((o) => o.type === 'client_revendeur')
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field label="Type de relation">
              <Select
                value={typeRelation}
                onChange={(e) => setTypeRelation(e.target.value as Organisation['type'])}
              >
            ...
            <Field label="Revendeur rattaché" hint="uniquement si client d’un revendeur">
              <Select
                value={revendeur}
                disabled={typeRelation !== 'client_revendeur'}
                onChange={(e) => setRevendeur(e.target.value)}
              >
