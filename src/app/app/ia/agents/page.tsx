...
export default function Agents() {
  ...
  const [selection, setSelection] = useState(agents[0]?.id ?? '')
  const agent: AgentIA | undefined = agents.find((a) => a.id === selection) ?? agents[0]
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        titre="Agents"
        sousTitre="Un agent, c’est un rôle, une consigne, un modèle, des outils et des limites. Tout le reste — la mémoire, les reprises, la trace — est le travail de la plateforme. Chaque enregistrement crée une version, et toute version se rejoue contre son jeu d’épreuves avant publication."
        actions={
          <GatedAction autorise={peutEcrire} message={refus('ia.agent.write')}>
            <ButtonLink href="/app/ia/agents/new">
              <Plus size={14} />
              Créer un agent
            </ButtonLink>
          </GatedAction>
      ...
      {agents.length === 0 ? (
        <EmptyState
          ...
          action={{ libelle: 'Créer un agent', href: '/app/ia/agents/new' }}
          actionSecondaire={{ libelle: 'Voir les outils disponibles', href: '/app/ia/outils' }}
        />
      ) : (
        <>
            ...
            {agents.map((a) => (
              <button
                ...
                className={cn(
                  ...
                  agent?.id === a.id ? 'border-p-700' : 'border-g-300 hover:border-p-400',
                )}
              >
                <div className="flex items-start justify-between gap-2">
                  <span className="flex min-w-0 items-center gap-2.5">
                    <SolutionLogo initiales={a.initiales} teinte={a.teinte} size="md" />
                    <span className="min-w-0">
                      <span className="block truncate text-[13px] font-bold text-ink">{a.nom}</span>
                      <span className="block text-[11px] text-g-500">
                        {TYPE_AGENT_LABEL[a.type]}
                      </span>
                    </span>
                  </span>
                  <Badge tone={TON_STATUT[a.statut]} dot size="sm">
                    {LIBELLE_STATUT[a.statut]}
                  </Badge>
                </div>
                <p className="mt-2.5 line-clamp-2 text-[12px] leading-relaxed text-g-500">
                  {a.role}
                </p>
                <div className="mt-3 grid grid-cols-2 gap-2 border-t border-g-100 pt-2.5">
                  <span className="block">
                    <MicroLabel>Échanges 7 j</MicroLabel>
                    <span className="tnum block text-[12.5px] font-semibold text-ink">
                      {num(a.metriques.conversations7j)}
                    </span>
                  </span>
                  <span className="block">
                    <MicroLabel>Épreuves</MicroLabel>
                    <span
                      className={cn(
                        'tnum block text-[12.5px] font-semibold',
                        a.epreuves.reussis / a.epreuves.cas < 0.8 ? 'text-warn' : 'text-ink',
                      )}
                    >
                      {a.epreuves.reussis}/{a.epreuves.cas}
                    </span>
                  </span>
                </div>
                <div className="mt-2.5 flex flex-wrap gap-1.5">
                  <Badge tone="neutral" size="sm">
                    {a.outils.length} outil{a.outils.length > 1 ? 's' : ''}
                  </Badge>
                  {a.connaissances.length > 0 && (
                    <Badge tone="neutral" size="sm">
                      {a.connaissances.length} base{a.connaissances.length > 1 ? 's' : ''}
                    </Badge>
                  )}
                  {a.humainDansLaBoucle && (
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <Card>
                <CardHeader
                  ...
                  actions={
                    <ButtonLink href="/app/ia/outils" variant="ghost" size="sm">
                      <Wrench size={13} />
                      Catalogue
                    </ButtonLink>
                  }
              ...
              <div className="space-y-4">
                ...
                <Card>
                  <CardHeader
                    ...
                    actions={
                      <ButtonLink href="/app/ia/routage" variant="ghost" size="sm">
                        <ShieldCheck size={13} />
                        Régler
                      </ButtonLink>
                    }
