...
import { JOBS_PLATEFORME, TACHES_PROVISIONING } from '@/lib/mock'
...
export async function generateMetadata({
  ...
  const job = JOBS_PLATEFORME.find((j) => j.id === id)
  return { title: job ? `Suivi · ${job.label}` : 'Tâche introuvable' }
}

export default async function PageSuiviTache({
  params,
}: {
  params: Promise<{ id: string }>
}) {
  ...
  const autres = JOBS_PLATEFORME.filter((j) => j.id !== id && j.orgId === job.orgId).slice(0, 4)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <ButtonLink href="/app/services" variant="secondary" iconBefore={<ArrowLeft size={14} />}>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_320px]">
        <div className="min-w-0 space-y-4">
          ...
          {job.statut === 'failed' && (
            <Callout
              ton="err"
              titre="Ce job a échoué et a été annulé proprement"
            >
